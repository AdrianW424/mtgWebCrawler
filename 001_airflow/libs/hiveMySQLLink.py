...
def main(date):
    """ main function to be runned by the DAG
    ...
    ret = htm.exchangeHiveToMySQL(hiveCursor, mysqlCursor, f"SELECT id, image_id, name, type FROM default.cards_basics WHERE partition_year={year} AND partition_month={month} AND partition_day={day}", "INSERT INTO mtg.cards_basics (id, image_id, name, type) VALUES (%s, %s, %s, %s)")